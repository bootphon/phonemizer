...
def test_bad_text():
    ...
    assert backend.phonemize([text], default_separator, True) == ["həloʊ wɜːld"]
...
def test_stress():
    ...
    assert backend.phonemize(["hello world"], default_separator, True) == [
        "həloʊ wɜːld"
    ]
...
@pytest.mark.skipif(
    (
        ...
        or not EspeakBackend.is_supported_language("ar")
    ),
...
@pytest.mark.skipif(
    "PHONEMIZER_ESPEAK_LIBRARY" in os.environ, reason="cannot modify environment"
)
def test_path_good():
...
@pytest.mark.skipif(
    not EspeakBackend.is_espeak_ng(), reason="tie only compatible with espeak-ng"
)
@pytest.mark.parametrize(
    "tie, expected",
    [(False, "dʒ_æ_k_i_ tʃ_æ_n_ "), (True, "d͡ʒæki t͡ʃæn "), ("8", "d8ʒæki t8ʃæn ")],
)
def test_tie_simple(caplog, tie, expected):
    backend = EspeakBackend("en-us", tie=tie)
    assert (
        backend.phonemize(["Jackie Chan"], separator=Separator(word=" ", phone="_"))[0]
        == expected
    )
    ...
    if tie:
        ...
        assert (
            "cannot use ties AND phone separation, ignoring phone separator" in messages
        )
...
def test_tie_utf8():
    ...
    assert backend.phonemize(["tu aimes le football"]) == ["ty ɛm lə (͡e͡n)fʊtbɔːl(͡f͡r) "]

    assert backend.phonemize(["bonjour apple"]) == ["bɔ̃ʒuʁ (͡e͡n)apə͡l(͡f͡r) "]
